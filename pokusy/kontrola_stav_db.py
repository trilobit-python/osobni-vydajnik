...
def print_query_result(p_db, p_query, msg=None):
    ...
    with pd.option_context('display.max_rows', 10, 'display.max_columns', None, 'display.width', 1000):  # more options can be specified also
        if not df.empty:
            xprint(df)
    xprint('--')
...
def check_ora_database_name(db_in_tns, pwd):
    xprint(db_in_tns)
    connection = cx_Oracle.connect(f'nemecd/{pwd}@'+db_in_tns)
    ...
    for env in ['NOE', 'ACC', 'PRO']:
        for db_inst in ['OEICM01', 'OEICM02', 'OE', 'OEPUG', 'OEMIG']:
            ...
            if bExistuje:
                ...
                if not (env == 'ACC' and db_inst == 'OEPUG'):
                    if not (env == 'PRO' and db_inst == 'OEPUG'):
...
if __name__ == '__main__':
    ...
    target_dir_name = os.path.abspath(os.path.join(os.getcwd(), ".."))
