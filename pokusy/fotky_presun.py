...
nazvy_poli_statistiky = [CHYBA, ZKONTROLOVANO, V_PORADU, NA_PRESUN, PRESUNUTO]
...
def doporuceny_nazev_souboru_bez_pripony(p_cas_vzniku_z_metadat: datetime, p_cas_vzniku_z_nazvu: datetime,
                                         p_cas_vzniku_souboru: datetime_str, p_format_nazvu: str = '%Y-%m-%d %H-%M-%S'):
...
def zpracuj_soubor(p_root, p_fname, celkova_statistika, p_presun, p_cil_koren):
    ...
    if cas_vzniku_z_nazvu is None:
        cas_vzniku_z_nazvu = datum_vzniku_z_nazvu(full_path)
        print(f'Chyba nelze určit čas u souboru: {full_path}')
        celkova_statistika.update([CHYBA])
        return
    ...
    if p_presun:
        ...
        print(f'{full_path} -> {kam_cela_cesta_soubor_kam}')
        celkova_statistika.update([NA_PRESUN])


def main(p_dir, p_cil_koren, p_presun):
    ...
    for root, dirs, files in os.walk(source_dir_name, topdown=False):
        for fname in files:
            ...
            if pripona in PODPOROVANE_PRIPONY:
                zpracuj_soubor(root, fname, celkova_statistika, p_presun, p_cil_koren)
...
if __name__ == '__main__':
    ...
    parser.add_argument('dir', help='Zdroj. adresář pro přesun fotek', default='.')
    parser.add_argument('cil_koren', help='Cíl. adresář kam se bude přesouvat')
    parser.add_argument('-p', '--presun', action='store_true', help='Opravdu přesunout, jinak pouze výpis')
    config = parser.parse_args()
    print(config)
    main(config.dir, config.cil_koren, config.presun)
