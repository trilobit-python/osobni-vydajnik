...
def zpracuj_soubor(p_root, p_fname, celkova_statistika, p_prejmenuj):
    ...
    if (vytvoreno - zmeneno).total_seconds() > 1:  # rozdíl více než 1 vteřina
        print(f'{p_fname} Oprava vytvořeno ({popis_datetime_tz(vytvoreno)}) > změněno ({popis_datetime_tz(zmeneno)})')
...
def main(p_config):
    ...
    print(f'Příznaky nastavení : {str(prejmenuj)}')
...
if __name__ == '__main__':
    ...
    parser.add_argument('dir', help='kořenový adresář stromu pro přejmenování souborů')
    parser.add_argument('-p', '--prejmenuj', action='store_true', help='Opravdu přejmenuje, jinak pouze výpis')
